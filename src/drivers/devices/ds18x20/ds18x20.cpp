...
/**
 * @defgroup DS18X20_Private_Constants DS18x20 Private Constants
 * @{
 */
...
/** @brief CRC8 polynomial for DS18x20 scratchpad validation (Dallas/Maxim algorithm) */
#define DS18X20_CRC8_POLY     0x8C
/** @brief Number of bytes to include in CRC calculation */
#define DS18X20_CRC8_BYTES    8
/** @brief Size of edge capture buffer for presence detection */
#define CAPTURE_BUF_SIZE      2
/** @brief Total length of DS18x20 scratchpad in bytes */
#define DS18X20_SCRATCHPAD_LEN   9
/** @brief Standard 8 bits per byte */
#define DS18X20_BITS_PER_BYTE    8
/** @brief Total number of bits in DS18x20 scratchpad */
#define DS18X20_SCRATCHPAD_BITS (DS18X20_SCRATCHPAD_LEN * DS18X20_BITS_PER_BYTE)
...
#define DS18X20_DMA_TRANSFERS   16
...
void DS18X20::detect_sensor_type() {
    // DS18S20 не имеет конфигурационного регистра — scratchpad[4] = 0xFF
    m_family = (m_ctx.scratchpad[4] == 0xFF) ? Family::DS18S20 : Family::DS18B20;
}

void DS18X20::ForceUpdateEvent(TIM_TypeDef *tim) {
...
/**
 * @defgroup DS18X20_Private_Functions DS18x20 Private Functions
 * @{
 */

/**
 * @brief Weak implementation for DS18x20 LED control - provides visual feedback
 ...
 */
void ds18x20_led_control(unsigned action) {
    if (action) {
...
/**
 * @brief Weak implementation for DS18x20 temperature ready callback - handles temperature display
 * @param[in] temp Temperature value in tenths of degrees Celsius, or error code
 */
#if defined ELAPSED_TIME
void ds18x20_temp_ready(int16_t temp, uint32_t t) {
#else

void ds18x20_temp_ready(int16_t temp) {
#endif
    if (temp == DS18X20::ErrorStatus::TEMP_ERROR_NO_SENSOR) { // No sensor detected error - enqueue error message
        if (app.uart) app.uart->write_str("DS18x20 error: no sensor detected.\r\n");
    } else if (temp == DS18X20::ErrorStatus::TEMP_ERROR_CRC_FAIL) { // CRC check failed error - enqueue error message
        if (app.uart) app.uart->write_str("DS18x20 error: CRC check failed.\r\n");
    } else if (temp == DS18X20::ErrorStatus::TEMP_ERROR_GENERIC) { // Generic error - enqueue error message
        if (app.uart) app.uart->write_str("DS18x20 error: generic failure.\r\n");
 ...
 */
void DS18X20::decode_scratchpad() {
    // Process each byte in the scratchpad (9 bytes total)
    for (unsigned byte = 0; byte < DS18X20_SCRATCHPAD_LEN; ++byte) {
        const unsigned bit_start = byte * DS18X20_BITS_PER_BYTE;
        // Process each bit in the byte (8 bits per byte)
        for (unsigned bit = 0; bit < DS18X20_BITS_PER_BYTE; ++bit) {
 ...
 */
int16_t DS18X20::decode_temperature() {
...
/**
 * @brief Verify presence of DS18x20 sensor by checking reset pulse timing
...
/**
 * @brief Transmit command sequence to DS18x20 using DMA
 ...
 */
void DS18X20::send_command(const uint8_t *cmd) {
    // Configure timer for command transmission using DMA
    TIM1->RCR = DS18X20_DMA_TRANSFERS - 1;   // Number of repetitions (16 transfers)
    ...
    DMA1_Channel4->CNDTR = DS18X20_DMA_TRANSFERS;    // Number of transfers
    // Enable DMA with memory increment
...
/**
 * @brief Read scratchpad data from DS18x20 using timer capture and DMA
...
void DS18X20::read_data() {
    ...
    TIM1->RCR = DS18X20_SCRATCHPAD_BITS - 1; // Number of repetitions (72 bits)
    TIM1->ARR = ONE_PULSE + ZERO_PULSE + 1;  // Total bit slot time (62µs)
    ...
    DMA1_Channel3->CNDTR = DS18X20_SCRATCHPAD_BITS;                    // Number of transfers (72 bits)
...
/**
 * @defgroup DS18X20_Public_Functions DS18x20 Public Functions
 * @{
 */

// Методы-действия для состояний FSM
void DS18X20::action_idle() {
#if defined ELAPSED_TIME
...
void DS18X20::action_decode() {
    ...
    if (m_ctx.scratchpad[8] == check_scratchpad_crc()) {
        ...
        ds18x20_temp_ready(decode_temperature(), DWT->CYCCNT - elapsed_time);
    } else {
        // CRC invalid - report error
        ds18x20_temp_ready(ErrorStatus::TEMP_ERROR_CRC_FAIL, DWT->CYCCNT - elapsed_time);
    }
#else
    if (m_ctx.scratchpad[8] == check_scratchpad_crc()) {
        // CRC valid - decode and report temperature
        ds18x20_temp_ready(decode_temperature());
    } else {
        // CRC invalid - report error
        ds18x20_temp_ready(ErrorStatus::TEMP_ERROR_CRC_FAIL);
    }
#endif
...
/**
 * @brief Initialize DS18x20 driver - configure clocks and peripherals
 */
void DS18X20::init() {
...
#if defined ELAPSED_TIME
        ds18x20_temp_ready(ErrorStatus::TEMP_ERROR_GENERIC, DWT->CYCCNT - elapsed_time);
#else
