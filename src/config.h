...
/** Включает PID-регулятор нагревателя и физический PWM-выход нагрева.
 *
 *  true  — нагреватель инициализируется, Controller рассчитывает мощность
 *          через PID и подаёт её на PWM (обычный режим термостата).
 *  false — драйвер нагревателя не создаётся (app.heater остаётся nullptr),
 *          PID не считается. Устройство работает как термометр/индикатор
 *          без управления нагревом — удобно для стендов без подключённого
 *          нагревателя или для экономии ресурсов на этапе отладки.
 */
static constexpr bool HEATER_CONTROL_ENABLED = false;
